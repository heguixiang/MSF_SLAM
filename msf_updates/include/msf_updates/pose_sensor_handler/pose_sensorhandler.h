...
#include <sensor_fusion_comm/DoubleArrayStamped.h>   //add by solomon
...
class PoseSensorHandler : public msf_core::SensorHandler<
 ...
 private:
  ...
  /**       solomon add      **/
  ros::Subscriber subEulerGroundTruth_;
  ros::Publisher  pubEulerGroundTruth_;

  ros::Subscriber subEulerEKF_;
  ros::Publisher  pubEulerEKF_;
  
  ros::Subscriber subEulerSLAM_;
  ros::Publisher  pubEulerSLAM_;

  float eulerError[3] = {0};
  float dynamicScale = 1.0f;
  void Qualternion2EulerGroundTruthCallback(
      const geometry_msgs::TransformStampedConstPtr & msg);
  void Qualternion2EulerEKFCallback(
      const sensor_fusion_comm::DoubleArrayStampedConstPtr & msg);
  void Qualternion2EulerSLAMCallback(
      const geometry_msgs::TransformStampedConstPtr & msg);

  // add for compute measurement covariance 
#if 0
  ros::Subscriber subMeasCovariance_;
  ros::Publisher  pubMeasCovariance_;
  float lastestMeasValue[7] = {0}; //translation: x,y,z  rotation: w,x,y,z
  int   countNum = 0;
  float firstMeasValue[3] = {0};
  bool firstMeasFlag = 0;
  float measResidualValue[2] = {0};
  void GetMeasCallback(
